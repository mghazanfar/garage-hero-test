            ...
            <div className="p-4 flex flex-col h-full">
                <div className="space-y-4 flex-1">
                    <div className="space-y-2">
                        <button
                            ...
                            className={`flex items-center  w-full px-2 py-2 text-sm font-medium text-[#111928] bg-[#f3f4f6] rounded-md ${isClosed ? "justify-center" : "justify-between"}`}
                        >
                ...
                <div className="relative group mb-20">
                    <div
                        onClick={logout}
                        className={`flex items-center ${isClosed ? "justify-center" : ""
                            } px-2 py-2 text-sm font-medium text-[#6b7280] hover:bg-[#f3f4f6] rounded-md cursor-pointer`}
                    >
                        <LogOutIcon className="w-4 h-4 min-w-4" />
                        {!isClosed && <span className="ml-2">Logout</span>}
                    </div>
                    {isClosed && (
                        <div className="absolute left-full ml-2 px-2 py-1 bg-gray-800 text-white text-xs rounded opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-opacity duration-300 whitespace-nowrap">
                            Logout
